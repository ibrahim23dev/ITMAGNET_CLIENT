    ...
    return (
      <DashboardShell activePath="/tickets" title="Loading Ticket...">
        <div className="flex flex-col items-center justify-center py-24 space-y-4">
          <div className="h-12 w-12 animate-spin border-4 border-slate-200 border-t-primary"></div>
  ...
  if (!ticket) {
    return (
      <DashboardShell activePath="/tickets" title="Ticket Not Found">
        <div className="p-12 text-center border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900">
           ...
           <Button onClick={() => router.push('/tickets')} className="mt-8">Back to Ticket Hub</Button>
        </div>
      </DashboardShell>
    );
  }
  ...
  return (
    <DashboardShell activePath="/tickets" title={ticket.ticketNumber || `Ticket #${ticketId.slice(-6).toUpperCase()}`}>
      <div className="space-y-8 pb-20">
        {/* Header Section */}
        <div className="group relative border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-8 lg:p-10 overflow-hidden">
           ...
           <div className="relative z-10 flex flex-col lg:flex-row gap-8 justify-between">
              <div className="space-y-4 flex-1">
                ...
                <h1 className="text-3xl lg:text-4xl font-extrabold tracking-tight text-slate-900 dark:text-white leading-tight">
                 ...
                 <div className="flex flex-wrap gap-4 items-center text-sm font-medium">
                   <div className="flex items-center gap-2 px-3 py-1.5 bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400">
              ...
              <div className="flex flex-col gap-4 min-w-[200px]">
                 <div className="flex gap-2 justify-end">
                    <Badge variant={ticket.status === 'open' ? 'success' : 'neutral'} className="h-8 px-3 uppercase text-[10px] tracking-widest font-bold">
                       {ticket.status}
                    </Badge>
                    <Badge variant={ticket.priority === 'high' || ticket.priority === 'critical' ? 'danger' : 'warning'} className="h-8 px-3 uppercase text-[10px] tracking-widest font-bold">
                       {ticket.priority}
                    </Badge>
                 </div>
                 <div className="flex flex-wrap gap-2 justify-end">
                    <Button variant="outline" size="sm" className="h-10 border-slate-200 dark:border-slate-800">
                       <MoreHorizontal className="h-4 w-4" />
                    </Button>
                    <Button 
                      onClick={() => handleStatusUpdate(ticket.status === 'resolved' ? 'open' : 'resolved')}
                      className={cn(
                        "h-10 px-6 font-bold text-sm transition-all",
           ...
           <div className="mt-10 p-6 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800">
        ...
        <div className="grid gap-8 xl:grid-cols-[1.4fr_0.6fr]">
          <div className="space-y-8">
            ...
            <div className="space-y-6">
               <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2 text-xs font-bold text-slate-400 bg-slate-100 dark:bg-slate-900 px-3 py-1.5">
          ...
          <div className="space-y-8">
            ...
            <div className="p-8 bg-slate-900 text-white border border-white/5 relative overflow-hidden group">
               <div className="absolute bottom-0 left-0 w-32 h-32 bg-primary/20 blur-3xl -ml-16 -mb-16"></div>
               <div className="relative z-10 space-y-6">
                  ...
                  <div className="space-y-4">
                     ...
                     <div>
                        ...
                        <p className="text-sm font-semibold">{ticket.assignee?.name || 'Unassigned (Auto-pilot)'}</p>
                     </div>
                     <div className="h-[1px] bg-white/10 w-full"></div>
                     <div>
                  ...
                  <Button className="w-full h-12 bg-white text-slate-950 font-bold hover:bg-slate-100">
                    Assign to Me
                  </Button>
               </div>
            </div>
            ...
            {/* SLA Timer */}
            <div className="p-8 border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950">
               <div className="flex flex-col items-center text-center space-y-3">
                  ...
                  <div className="w-full h-1 bg-slate-100 dark:bg-slate-900 overflow-hidden mt-2">
...
function Badge({ children, variant = "outline", className = "" }: { children: React.ReactNode, variant?: string, className?: string }) {
  ...
  return (
    <div className={cn("inline-flex items-center justify-center text-xs font-semibold ring-offset-background transition-colors", styles[variant], className)}>
      {children}
    </div>
  );
}
